...
#include "utils/panda_2_gen.h"

// constexpr std::string path_yaml_DH_REG = "../panda_2s/franka/generatedFiles/inertial_REG_stored";
// constexpr std::string path_copy_DH_REG = "../panda_2s/franka/generatedFiles/inertial_REG_stored_copy";
...
namespace thunder_ns{
	// thunder_panda_2::thunder_panda_2(){};

	// thunder_panda_2::thunder_panda_2(const int nj_):num_joints(nj_){ 
	// 	resizeVariables();
	// }
	...
	// void thunder_panda_2::init(const int nj_){
	// 	num_joints = nj_;
	// 	resizeVariables();
	// }

	void thunder_panda_2::resizeVariables(){
	...
	// void thunder_panda_2::setArguments(const Eigen::VectorXd& q_,const Eigen::VectorXd& dq_){
	...
	// void thunder_panda_2::setArguments(const Eigen::VectorXd& q_){
	...
	void thunder_panda_2::computeReg_gen(){
	...
	void thunder_panda_2::computeMass_gen(){
	...
	void thunder_panda_2::computeCoriolis_gen(){
	...
	void thunder_panda_2::computeGravity_gen(){
	...
	void thunder_panda_2::computeJac_gen(){
	...
	void thunder_panda_2::computeDotJac_gen(){
	...
	void thunder_panda_2::computePinvJac_gen(){
	...
	void thunder_panda_2::computePinvJacPos_gen(){
	...
	void thunder_panda_2::computeDotPinvJac_gen(){
	...
	void thunder_panda_2::computeDotPinvJacPos_gen(){
	...
	void thunder_panda_2::computeKin_gen(){
	...
	Eigen::MatrixXd thunder_panda_2::getReg(){computeReg_gen(); return reg_gen;};
	/* Get regressor matrix */
	Eigen::MatrixXd thunder_panda_2::getMass(){computeMass_gen(); return mass_gen;};
	/* Get regressor matrix */
	Eigen::MatrixXd thunder_panda_2::getCoriolis(){computeCoriolis_gen(); return coriolis_gen;};
	/* Get regressor matrix */
	Eigen::MatrixXd thunder_panda_2::getGravity(){computeGravity_gen(); return gravity_gen;};
	/* Get jacobian matrix */
	Eigen::MatrixXd thunder_panda_2::getJac(){computeJac_gen(); return jac_gen;};
	/* Get derivative of jacobian matrix */
	Eigen::MatrixXd thunder_panda_2::getDotJac(){computeDotJac_gen(); return dotJac_gen;};
	/* Get pseudo-inverse jacobian matrix */
	Eigen::MatrixXd thunder_panda_2::getPinvJac(){computePinvJac_gen(); return pinvJac_gen;};
	/* Get derivative of pseudo-inverse jacobian matrix only position */
	Eigen::MatrixXd thunder_panda_2::getPinvJacPos(){computePinvJacPos_gen(); return pinvJacPos_gen;};
	/* Get derivative of pseudo-inverse jacobian matrix */
	Eigen::MatrixXd thunder_panda_2::getDotPinvJac(){computeDotPinvJac_gen(); return dotPinvJac_gen;};
	/* Get derivative of pseudo-inverse jacobian matrix only position */
	Eigen::MatrixXd thunder_panda_2::getDotPinvJacPos(){computeDotPinvJacPos_gen(); return dotPinvJacPos_gen;};
	/* Get regressor matrix */
	Eigen::MatrixXd thunder_panda_2::getKin(){computeKin_gen(); return kin_gen;};

	// Other functions
	void thunder_panda_2::fillInertialYaml(int num_joints, YAML::Emitter &emitter_, std::vector<LinkProp> &links_prop_, std::vector<std::string> keys_){
		YAML::Node control;
	...
	void thunder_panda_2::transformBodyInertial(std::vector<double> d_i, std::vector<double> rpy_i, const LinkProp body_urdf, LinkProp &body){
        Eigen::Vector3d OuGi; 
        Eigen::Vector3d OiGi;
    ...
    void thunder_panda_2::mergeBodyInertial(const LinkProp body1, const LinkProp body2, LinkProp &newBody){
    ...
    Eigen::Matrix3d thunder_panda_2::hat(const Eigen::Vector3d v){
        Eigen::Matrix3d vhat;
    ...
    Eigen::Matrix3d thunder_panda_2::rpyRot(const std::vector<double> rpy){
    ...
    Eigen::Matrix3d thunder_panda_2::createI(const std::vector<double> parI){
        Eigen::Matrix3d I;
        I(0, 0) = parI[0];
        I(0, 1) = parI[1];
        I(0, 2) = parI[2];
