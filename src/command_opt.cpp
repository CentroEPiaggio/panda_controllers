...
Eigen::VectorXd dq_curr;
Eigen::VectorXd ddq_curr;
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        for(int i = 0; i < 7; ++i){
            x[i] = ddq_curr(i);
            lb[i] = ddq_curr(i)-0.05;
            ub[i] = ddq_curr(i)+0.05;
            udata.q[i] = q_curr(i);
...
void jointsCallback(const panda_controllers::udata::ConstPtr& msg){
    ...
    Eigen::VectorXd dq_tmp;
    Eigen::VectorXd ddq_tmp;
    Eigen::VectorXd H_tmp;
    ...
    dq_tmp[0] = msg->dot_q_curr[0];
	dq_tmp[1] = msg->dot_q_curr[1];
	dq_tmp[2] = msg->dot_q_curr[2];
	dq_tmp[3] = msg->dot_q_curr[3];
	dq_tmp[4] = msg->dot_q_curr[4];
	dq_tmp[5] = msg->dot_q_curr[5];
	dq_tmp[6] = msg->dot_q_curr[6];

    ddq_tmp[0] = msg->ddot_q_curr[0];
	ddq_tmp[1] = msg->ddot_q_curr[1];
	ddq_tmp[2] = msg->ddot_q_curr[2];
	ddq_tmp[3] = msg->ddot_q_curr[3];
	ddq_tmp[4] = msg->ddot_q_curr[4];
	ddq_tmp[5] = msg->ddot_q_curr[5];
	ddq_tmp[6] = msg->ddot_q_curr[6];

    for(int i = 0; i<70; ++i){
        H_tmp.segment(i*PARAM, PARAM) << msg->H_stack[i*PARAM], msg->H_stack[i*PARAM+1], msg->H_stack[i*PARAM+2], msg->H_stack[i*PARAM+3], msg->H_stack[i*PARAM+4], msg->H_stack[i*PARAM+5], msg->H_stack[i*PARAM+6], msg->H_stack[i*PARAM+7], msg->H_stack[i*PARAM+8], msg->H_stack[i*PARAM+9];
    }
