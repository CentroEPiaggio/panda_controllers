...
void slow_down  (const double dt_, const vec3d p0, panda_controllers::desTrajEE &msg);
...
int main(int argc, char **argv)
{
    ...
    if (!node_handle.getParam("lissajous/ampX", ampX) ||
        ...
        !node_handle.getParam("break_time", break_time) ||
...
/*     cout<<"period: "<<period<<endl; */
	...
	while (ros::ok()){
        ...
        if (!start){
            ...
            dt = t.toSec() - t_start;
            if(break_time>0.0 && duration-dt<=break_time && flag_traj!=3){
                traj_ptr = slow_down;}
        }else if(!end_motion){
            std::cout<<"\nsto per bloccarmi\n"<<std::endl;
            std::cout<<msg_cartesian.position.x<<" "<<msg_cartesian.position.y<<" "<<msg_cartesian.position.z<<std::endl;
            std::cout<<msg_cartesian.velocity.x<<" "<<msg_cartesian.velocity.y<<" "<<msg_cartesian.velocity.z<<std::endl;
            std::cout<<msg_cartesian.acceleration.x<<" "<<msg_cartesian.acceleration.y<<" "<<msg_cartesian.acceleration.z<<std::endl;
...
void slow_down(const double dt_, const vec3d p0, panda_controllers::desTrajEE &msg){
    ...
    vec3d vel_0, acc_0;
    vec3d pos_t, vel_t, acc_t;
    vec3d coeff1, coeff2;
    vec3d add1, add2, add3;
    double delta_t;

    delta_t = break_time-(duration - (dt_));
    vel_0 = velocity_t;
    acc_0 = acceleration_t;
    coeff1 = (3*vel_0+2*acc_0)/3;
    coeff2 = (-2*vel_0-acc_0)/4;

    add1 = vel_0*(delta_t/break_time) - acc_0/2*(pow(delta_t/break_time,2)) - coeff1*(pow(delta_t/break_time,3)) - coeff2*(pow(delta_t/break_time,4));
    add2 = acc_0*(delta_t/break_time) - 3*coeff1*(pow(delta_t/break_time,2)) - 4*coeff2*(pow(delta_t/break_time,3));
    add3 = - 6*coeff1*(delta_t/break_time) - 12*coeff2*(pow(delta_t/break_time,2));

    pos_t = p0 + add1;
    vel_t = vel_0 + add2;
    acc_t = acc_0 + add3;
    
    std::cout<<"slow down delta_t\n"<<delta_t<<std::endl;    
    std::cout<<"slow down pos_t\n"<<pos_t<<std::endl;    
    std::cout<<"slow down vel_t\n"<<vel_t<<std::endl;    
    std::cout<<"slow down acc_t\n"<<acc_t<<std::endl;    
    std::cout<<"================\n"<<std::endl;

    msg.position.x = pos_t(0);
    msg.position.y = pos_t(1);
    msg.position.z = pos_t(2);

    msg.velocity.x = vel_t(0);
    msg.velocity.y = vel_t(1);
    msg.velocity.z = vel_t(2);

    msg.acceleration.x = acc_t(0);
    msg.acceleration.y = acc_t(1);
    msg.acceleration.z = acc_t(2);
}
