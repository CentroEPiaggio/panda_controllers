...
void ProjectImpedanceControllerPos::update(  const ros::Time& /*time*/,
	...
	tau_or = ja_or.transpose() * ( -I3*error_or*PD_K_OR - I3*derror_or*PD_D_OR );
	...
	wrench_task_or << ( - PD_K_OR * I3 * error_quat               // proportional term
                 		- PD_D_OR * I3 * derror_quat );               // derivative term

	//tau_or << N * ( jacobian.bottomLeftCorner(3,7).transpose() * wrench_task_or );

	// FUNZIONANTE
	//tau_or << jacobian.bottomLeftCorner(3,7).transpose() * wrench_task_or;
