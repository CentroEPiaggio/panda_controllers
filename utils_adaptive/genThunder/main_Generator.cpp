...
int main(){
    ...
    //FrameOffset Ln_to_EE({-M_PI_4,0,0},{0,0,0.1034},{0,0,-9.80});
    FrameOffset Ln_to_EE({0,0,0},{0,0,0},{0,0,-9.80});

    /* RobKin, RobReg, SelfMoveCost object */;
    RobKinAdv kinrobot;
    RobReg regrobot;

    kinrobot.init(nj,jType,DH_table,base_to_L0,Ln_to_EE, 0.02);
    regrobot.init(nj,jType,DH_table,base_to_L0,Ln_to_EE);
    
    /* Get Casadi Functions */
    std::vector<casadi::Function> kin_vec, reg_vec, all_vec;
    ...
    if(all_vec.size()!=dim1+dim2) cout<<"Merge Error"<<endl;
