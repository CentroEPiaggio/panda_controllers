...
# Capsule–Sphere (Palla) -------------------------------------------------------
def dist_capsule_sphere(P0, P1, r_capsule, C, r_sphere):
    u = P1 - P0
    v = C - P0
    ...
    eps = 1e-12
    t_raw = ca.if_else(u_dot_u < eps, 0.0, v_dot_u / u_dot_u)
    t_clamped = clamp01(t_raw)
    P_closest = P0 + t_clamped * u
    
    diff = C - P_closest
    # Protezione: evita gradiente NaN quando sfera è esattamente sull'asse
    d_core = ca.sqrt(ca.dot(diff, diff) + eps)
    
    dist = d_core - (r_capsule + r_sphere)
    return dist, P_closest, C, t_clamped
...
def dist_capsule_capsule(A0, A1, rA, B0, B1, rB):
    ...
    diff = PA - PB
    eps_norm = 1e-6
    d = ca.sqrt(ca.dot(diff, diff) + eps_norm)
    return d - (rA + rB), PA, PB, s, t
...
def capsule_distance(shape_type, **kwargs):
    if shape_type == "capsule":
        ...
        d, P_capsule, C_sphere, t = dist_capsule_sphere(
            kwargs['P0'], kwargs['P1'], kwargs['r_capsule'],
            kwargs['C'], kwargs['r_sphere']
        )
        return d, {'p_capsule': P_capsule, 'p_obstacle': C_sphere, 't_capsule': t}
...
def test_capsule_distance():
    ...
    d_sphere, _, _, _ = dist_capsule_sphere(P0, P1, r, C_sym, r)
    
    print("Funzioni create con successo")
    print("dist_capsule_capsule:", d_capsule)
    print("dist_capsule_plane:", d_plane)
    print("dist_capsule_sphere:", d_sphere)
